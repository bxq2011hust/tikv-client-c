...
protected:
  void SetUp() override {
    ...
    auto fileChannel = Poco::AutoPtr<Poco::FileChannel>(
        new Poco::FileChannel("tikv-client.log"));
    ...
    Poco::Logger::root().setLevel(
        Poco::Message::PRIO_DEBUG); // PRIO_TRACE PRIO_DEBUG
...
TEST_F(TestWith2PCRealTiKV, bcosCommmit) {
  // Prewrite
  {
    ...
    BCOSTwoPhaseCommitter committer{test_cluster.get(), "a",
                                    std::move(mutations)};
    ...
    mutations2["e"] = "e";
    mutations2["f"] = "f";
    BCOSTwoPhaseCommitter committer2(test_cluster.get(), "a",
                                     std::move(mutations2));
  ...
  // Prewrite
  {
    ...
    for (size_t i = 0; i < loop; ++i) {
      ...
      for (size_t j = 0; j < commitSize; ++j) {
        keys.push_back("key________________________________" +
                       std::to_string(rand()));
        std::string value(64, 'a');
        value +=
            "value________________________________" + std::to_string(rand());
        value.resize(commitSize);
        values.push_back(std::move(value));
        mutations2[keys[j]] = values[j];
      }
      auto start = std::chrono::system_clock::now();
      BCOSTwoPhaseCommitter committer{test_cluster.get(), "a",
                                      std::move(mutations), 32768, 20};
      try {
        auto result = committer.prewriteKeys();
...
TEST_F(TestWith2PCRealTiKV, performance) {
  // Prewrite
  {
    ...
    size_t count = 0;
    for (; scanner.valid && scanner.key().rfind(kPrefix, 0) == 0;
         scanner.next()) {
      ++count;
    }
    ...
    BCOSTwoPhaseCommitter committer3{test_cluster.get(), "a",
                                     std::move(mutations3)};
